...
def calculate_line_curve_pattern(row, x_coord_line, y_coord_line, df_formula, x_shift, y_shift):
    ...
    for i in range(0, len(x_coord_line) - 1, 2):
        ...
        # если отклонений несколькоy_shift
        else:
            k = len(x_deviation[j])
...
def create_user_scheme_pattern(conn, user_param, list_id_detail, name_pattern, func_name):
    ...
    for index, row in df_line.iterrows():
        ...
        if df_line.loc[index, 'x_deviation'] == '':
            ...
            try:
                curve, points_curve = calculate_line_curve_pattern(row, x_coord_line, y_coord_line, df_formula, x_shift, y_shift)
            except ValueError:
                return "error_line"
